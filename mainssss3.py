...
#             # fix 3: explicit override - always block these classes
#             # regardless of what the (possibly noisy) local slope
#             # reads. Checked before the generic OBSTACLE_CLASSES
#             # test so the rule is visible and independent of it.
#             if label_int in FORCE_NON_TRAVERSABLE_CLASSES:
...
#             if label_int in OBSTACLE_CLASSES:
...
#             # fix 3: explicit override - road/parking/sidewalk are
#             # graded purely on slope once terrain is known.
#             if label_int in FORCE_TRAVERSABLE_CHECK_CLASSES:
...
#                 elif slope_degrees[
#                     gy, gx
#                 ] <= get_class_max_slope(label_int, max_slope):
...
#             if label_int not in TRAVERSABLE_CLASSES:
...
#             # Semantic-aware slope cutoff: any other traversable
#             # class (e.g. terrain, other-ground) gets its own
#             # tolerance (CLASS_MAX_SLOPE) instead of one blanket
#             # MAX_SLOPE for every ground type.
#             if slope_degrees[
#                 gy, gx
#             ] > get_class_max_slope(label_int, max_slope):
...
def build_cost_map(
    ...
    - Unknown cells (traversability_grid == 2)
      -> unknown_cost (blocked by default - the planner never
         routes through unmapped terrain unless you relax this)
    ...
    grid_height, grid_width = traversability_grid.shape
    ...
    for gy in range(grid_height):
        ...
        for gx in range(grid_width):
            ...
            trav = traversability_grid[gy, gx]

            if trav == 0:

                cost_grid[gy, gx] = obstacle_cost

            elif trav == 2:

                cost_grid[gy, gx] = unknown_cost

            else:

                label = int(semantic_grid[gy, gx])

                risk_weight = SEMANTIC_RISK_WEIGHTS.get(label, 1.5)

                slope_term = normalized_slope[gy, gx]

                if np.isnan(slope_term):

                    slope_term = 1.0

                cost_grid[gy, gx] = (1.0 + slope_term) * risk_weight
...
def a_star_search(
    ...
    def heuristic(gx, gy):
        ...
        return math.hypot(gx - gx_goal, gy - gy_goal)
...
def plan_path(
    ...
    to grid cells, runs A*, and converts the resulting cell path
    back to world-frame (x, y) coordinates for plotting.
    """
