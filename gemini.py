...
BIN_FILE = (
    ...
    r"\velodyne\000001.bin"
)

LABEL_FILE = (
    r"semantic_kitti_sample\dataset\sequences\00"
    r"\labels\000001.label"
)
...
GOAL_COORD = (20, 15)    # example goal, world (x, y) meters
...
def plan_path(
    ...
    return [grid_to_world(gx, gy, x_min, y_min, resolution) for gx, gy in cell_path]
...
def main():
    ...
    cost_grid = build_cost_map(
        ...
        max_slope=MAX_SLOPE, unknown_policy=UNKNOWN_TRAVERSAL_POLICY
    )
    passable_fraction = float(np.isfinite(cost_grid).mean())
    print(f"Cost map: {passable_fraction * 100:.1f}% of cells passable (unknown_policy='{UNKNOWN_TRAVERSAL_POLICY}')")
    ...
    if path_world is not None:
        print(f"Path found: {len(path_world)} waypoints, from {START_COORD} to {GOAL_COORD}")
    else:
